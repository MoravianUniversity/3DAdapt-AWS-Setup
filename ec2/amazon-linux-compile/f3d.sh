...
wget "https://www.vtk.org/files/release/${VTK_VERSION%.*}/VTK-$VTK_VERSION.tar.gz"
...
cd package
tar -czf ../../../vtk-"$VTK_VERSION"-"$(arch)"-linux-gnu.tar.gz --owner=0 --group=0 -- *
...
##### Build F3D #####
wget -O "f3d-$F3D_VERSION.tar.gz" "https://github.com/f3d-app/f3d/archive/refs/tags/v$F3D_VERSION.tar.gz"
tar -xzf "f3d-$F3D_VERSION.tar.gz" && cd f3d-$F3D_VERSION
mkdir build && cd build
cmake .. -GNinja -DF3D_MODULE_EXTERNAL_RENDERING=ON -DF3D_BINDINGS_PYTHON=ON -DF3D_PLUGIN_BUILD_EXODUS=OFF
cmake --build .
sudo cmake --install . >installed.txt
sudo cmake --install . --component sdk >installed-sdk.txt
